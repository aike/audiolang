...
Tone.jsを使う場合、npm install tone --save のようにします。これによりpackage.jsonのdependenciesにtoneが追加されます。
...
p5.jsの場合、npmやwebpackを使わない方がシンプルでProcessingらしい書き方になるので、それらを使わずに以下のindex.htmlのようにheadの中でp5.jsとp5.sound.jsを取得して利用するようにします。
...
p5.jsもシンプルに直感的に書くことができます。発音ノードのライフサイクルを気にする必要がありません。p5.jsではキャンバスのクリックに反応するプログラムが慣習的に多いのでAutoplay Policyも気になりません。
...
Web Audio APIが最初に登場したときは、本格的なオーディオプログラミングがそのまますぐに公開できるということで画期的に感じました。
その後、APIもJavaScriptをとりまく環境も進化していった結果、使いやすくなった点も多いですがシンプルさが失われていった部分もあります。また発音ノードのライフサイクルなどあまり直感的ではない仕様も気になります。
...
p5.jsやFaustなど、プログラミング言語をローカルにインストールせずにWeb上のエディタだけでオーディオ処理を試すことができる環境が増えてきたのも、Web Audio APIの重要な功績です。

一方で、実装がブラウザのデベロッパー依存であるため、細かい部分ではブラウザによって挙動が異なることもあります。今回のプログラムはすべてGoogle Chromeで確認しました。

なお、Web Audio APIはあくまでブラウザのAPIという位置づけのようで、Node.jsにオーディオ機能が搭載されることはなさそうです。
Node.js上にWeb Audio API相当のものを実現するライブラリはいくつか開発されましたが、いずれもNode.js公式のものではなく、今回調べたところ数年前から開発が止まっていて最新のWeb Audio APIに追従したものではないようでした。
